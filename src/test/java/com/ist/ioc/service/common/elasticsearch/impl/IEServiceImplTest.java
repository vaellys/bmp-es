...
public class IEServiceImplTest extends AbstractJUnit4SpringContextTests {
	...
	public void testDocumentHandler() {
		...
		try {
		    ...
		    for(int i = 0; i < 100; i++){
		        ...
		        map.put("id", 2221126 + i);
		        map.put("title", "张三 李四 王五");
    ...
    public void testDocumentHandler2() {
        ...
        try {
            ...
            Map<String, Object> map = new HashMap<String, Object>();
            map.put("id", 5);
            map.put("title", "crime and punishment");
    ...
    public void testDocumentHandlerScoreFunction() {
        ...
        try {
            ...
            for(int i = 600; i < 900; i++){
                Map<String, Object> map = new HashMap<String, Object>();
                map.put("id", 88894+i);
                map.put("title", "用一个示例解释下什么是相关性，例如有两篇文章都包含lucene这个词，第一篇文章共有10个词，lucene出现了5次，第二篇文章共有1000个词，lucene也出现了5次，按照默认的相关性考虑的话，第一篇文章相关性比第二篇文章要高，在相关性打分上，第一篇文章得到的分数也会比第二篇文章分数高。");
...
//          strList.add("content");
            ...
            Map<String, Object> documentSearchWithMatchPhrase = iesService.documentSearchWithMatchPhrase("appletest", "iphone", "test", hlFields , keywords, 1, 10);
...
//          strList.add("content");
            strList.add("content");
            List<String> hlFields = null;
            Map<String, Object> documentSearchWithMatchPhrase = iesService.documentSearchWithFuzzy("apple", "iphone", "title", hlFields , keywords, 1, 10);
            logger.debug(LogUtils.format("R", documentSearchWithMatchPhrase));
        } catch (Exception e) {
            e.printStackTrace();
            fail("----------构建索引失败----------");
...
//          strList.add("content");
            strList.add("content");
            List<String> hlFields = null;
            Map<String, Object> documentSearchWithMultiMatch = iesService.documentSearchWithMultiMatch("appletest", "iphone", new String[]{"test"}, hlFields , keywords, 1, 10);
